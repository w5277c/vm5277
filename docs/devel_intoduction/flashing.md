# Прошивка микроконтроллеров, обзор подходов:
...
*	Множественные графические обертки для avrdide
*	Консольный вариант:
```bash
...
| **Smart поддержка** | Нет | Да (прошиваются только измененные страницы) |
| **Поддержка платформ** | Одна утилита - одна платформа | Одна утилита - все платформы |
| **Используемая память Flash** | 0-1024 байт | 512-1024 байт |
