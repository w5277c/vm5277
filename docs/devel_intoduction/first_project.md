...
* Отредактируйте файл `pom.xml` указав необходимые параметры:
   - `<j8b.target>` — платформа:чип, пример `avr:atmega328`
...
* Создаем основной файл и напишем первую программу
	...
	class Main {
		public static void main() {
			System.setParam(RTOSParam.STDOUT_PORT, GPIO.PC0);
			cstr text="READY!\n";
			System.out(text);
		}
	}
...
* Пробуем ее скомпилировать
	...
	Если ошибок не обнаружено, то в директории нашего проекта должна была появиться диретория target:
	```bash
	target/
	├── Main.asm
	└── Main.dbg
	...
	1. `.include "devices/atmega328p.def"`
	2. `.equ CORE_FREQ = 16`
...
* Используем AVR ассемблер
	...
	Если ошибок не обнаружено, то в директории target появится новый файл `Main_cseg.hex`
