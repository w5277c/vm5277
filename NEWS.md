...
### Расширение runtime-среды и RTOS

**Назначение:** Создание полноценной многопоточной среды выполнения для AVR-микроконтроллеров.

**Новые компоненты runtime:**

- **Драйверы:**
  - `DHT11.j8b` — драйвер датчика температуры и влажности
  - `AnsiDisplay.j8b` — драйвер ANSI-дисплея с описанием необходимых интерфейсов

- **Многопоточность и диспетчеризация:**
  - `RTOSDispatecher.j8b` — диспетчер с методами блокировки
  - `RTOSPcint.j8b` — обработчик событий PCINT на верхнем уровне
  - `rtos.Timer` — программный таймер в многопоточном режиме (аналогичен `rtos.Thread`)
  - `rtos.Thread` — полная поддержка основного функционала (`start`, `stop`, `isAlive`, `isStopped`, `run`)
...
- **Расширение стандартных классов:**
  - `lang.Math` — методы для подсчёта CRC8
  - `lang.Primitive` — методы для raw-записи в примитивы `fixed`, `short`, `int`
  - `rtos.ANSI` — статические методы вывода в консоль с использованием ANSI-команд
  - `rtos.RTOSParam` — актуализирован
  - `rtos.System` — расширен: вывод в консоль, получение uptime, random, вывод карты DRAM, заполнение байтного массива
  - `util.CRC8` — статические методы вычисления CRC8 на верхнем уровне

**Автоимпорт и конфигурация:**

- Добавлены новые классы и интерфейсы в автоимпорт
- Добавлен `persist.cfg` — описание методов, код которых обязательно генерируется кодогенератором
- `native_binding.cfg` вынесен в RTOS конкретной платформы

---

### Модернизация RTOS для AVR

**Назначение:** Низкоуровневая реализация многопоточности, управления памятью и периферией для AVR.

**Бутлоадеры (`boot/`):**

- **bldr** — добавлена информация о портах STDIO, блокировка прерываний для вывода символа, оптимизация
- **bldrtiny** — актуализирован, теперь рабочая версия
- Остальные варианты бутлоадеров удалены

**Ядро (`core/`):**

- Переход с 3-блочной модели DRAM на 1-блочную (1 ячейка = 8 байт), багфиксинг
- Добавлен функционал подсчёта нагрузки на CPU
- Добавлена инициализация многопоточности и вызов диспетчера
- Добавлен функционал подсчёта uptime в микросекундах
- Добавлен функционал инициализации случайных чисел
- Добавлен функционал детектирования переполнения стека (черновой, не оттестирован)

**Диспетчер (`core/dispatcher`):**

- Практически полная реализация (не хватает блокировки потока по ресурсам)

**Новые функции ядра:**

- `core/uptime_us` — получение uptime в микросекундах
- `core/wait_ms` — перевод задачи в сон в многопоточном режиме вместо блокирующего ожидания

**Управление памятью (`dmem/`):**

- Переход на 1-блочную модель DRAM (1 ячейка = 8 байт), багфиксинг
- `dram_showmap` — диагностический функционал, отображение карты использования DRAM

**Другое:**

- **devices** — мелкие доработки
- **io** — переименованы функции и добавлены новые
- **j8b** — множественный багфиксинг, доработки, новый функционал (`new_class`, `new_thread`, `uptime`...)
- **math** — новый функционал (`crc8`, `reflect`, `shl`, `shr`)
- **mem** — новый функционал (`ram_copy`, `ram_popblk`, `ram_pushblk`)
- **stdio** — новый функционал: hex-вывод, `out_bytes`, диагностика (`out_heap`, `out_rambytes`, `out_stack`)
- **sys/prnd** — генератор псевдослучайных чисел
- **binding.cfg** — вынесен из runtime и доработан

---

### Расширение примеров

**Назначение:** Демонстрация новых возможностей платформы.

**Общие изменения:**

- Добавлены assembler actions для NetBeans
- Обновлены и расширены существующие примеры (3 примера временно поломаны, будут исправлены)
- Небольшие изменения свойств и профилей pom-файла родителя

**Новые примеры:**

| Пример | Описание |
|--------|----------|
| `ansi` | Вывод в консоль ANSI-команд |
| `bench_crc8` | Тест производительности подсчёта CRC8 |
| `cstr` | Вывод в консоль константных строк |
| `random` | Вывод в консоль псевдослучайных чисел |
| `rgb` | Изменение цвета RGB-светодиода |
| `rgbrand` | Изменение цвета RGB-светодиода на базе случайных чисел |
| `temp` | Вывод показаний DHT11 (температура/влажность) |
| `thread` | Демонстрация работы многопоточности |
| `timer1` | Демонстрация работы таймера |
| `usdelay` | Демонстрация получения uptime в микросекундах |

---

### Улучшение devs и common-компонентов

**Назначение:** Поддержка новых МК, улучшение компилятора, ассемблера, кодогенерации и инструментов прошивки.

**devs:**

- Добавлены сигнатуры микроконтроллеров (`signatures.txt`)
- Багфиксинг
- Добавлены новые файлы для МК: attiny13a, attiny2313a, attiny4313

**common/avr_assembler:**

- Багфиксинг
- Подсчёт обработанных строк

**common/avr_codegen (существенные изменения):**

- Использование новой константы-регистра `C0x02`
- Улучшена оптимизация
- Переход от множественных методов к общим: `cellToCells`, `cellsCpCells`
- Значительно упрощён и улучшен механизм сравнения (с учётом знакового типа)
- Поддержка присваивания с учётом различных комбинаций (в том числе с fixed)
- Приведение кода к единому виду (LittleEndian)

**common/common_lib:**

- Множество доработок, исправлений и улучшений

**common/compiler (существенные изменения):**

- Возможность задавать параметры через `RTOSParam`
- Компиляция на лету (вместо имени файла — строка кода)
- Конкатенация выражений в константную строку
- Обязательная кодогенерация методов из `persists.cfg`, если класс задействован
- Дополнительные проверки в семантике
- Резолвинг импортов в текущей папке
- Общий контекст в классе `Instance`
- Запрет использования оператора `new` (динамическая память) для устройств с RAM менее 256 байт
- Расширенный заголовок класса в HEAP для `Thread` и `Timer`, отдельные инициализации инстанса
- Поддержка примитивного типа `char` (беззнаковый, целочисленный, 8 бит)
- Расширенная передача статического результата условия
- Расширенная передача значений без участия аккумулятора — на основе флагов устройства
- Ввод сессионного ограничения размера аккумулятора в рамках цепочки выражений
- Ввод новых типов CGCells (`CONST`, `FLAG`)
- Доработка функционала для массивов
- Улучшена семантика `LiteralExpression`
- Существенно расширен и улучшен функционал `MethodCallExpression`
- Поддержка функций с инлайнингом кода непосредственно кодогенератором (например, `GPIO.setHi(n);`)
- Переход на универсальный метод `cellsToCells` в `AssignExpression`
- Переход на универсальный метод `cellsCpCells` в `ComparisonExpression`
- Исправлен механизм переиспользования регистров родительского объекта

**common/flasher:**

- Багфиксинг
- Учёт аппаратной перезагрузки PCB типа Arduino
- Обработка значений портов STDIO от бутлоадера
- Выход из интерактивного режима по Escape
- Режим определения МК

**common/ide-plugins:**

- **idea_plugin** — новый плагин с более богатым функционалом, чем в NetBeans
- **nb_plugin** — расширенная поддержка asm-файлов

**common/lsp-server:**

- Поддержка меток, констант, макросов, регистров и инструкций

**common/maven-tools:**

- **j8b-archtype** — добавлены assembler actions для NetBeans, небольшие изменения pom-файла
- **j8b-plugin** — существенные изменения кода, расширены возможности, добавлены новые опции (`j8b.optimization`, `j8b.strict`, `j8b.flashTool`, `j8b.bldrApiReuse`, `j8b.softReset`)
...
Проведена комплексная модернизация всей экосистемы, затрагивающая все уровни стека разработки:

- **RTOS и runtime** получили полноценную многопоточность, таймеры, драйверы и обработку исключений
- **Модель DRAM** упрощена с 3-блочной до 1-блочной
- **Компилятор и кодогенерация** кардинально улучшены — новые типы ячеек, универсальные методы, оптимизации
- **Примеры** расширены до 11 новых сценариев использования
- **Инструменты** (ассемблер, флашер, плагины, LSP-сервер) получили множественные улучшения

**Рекомендация:** Для начала работы с тулкитом лучше всего использовать ATmega328p (например, Arduino Uno) или ATmega168p.

### Следующие шаги
- Множественные мелкие доработки и багфиксинг
- Багфиксинг примеров
- Акутализация документации
- Публикация на Habr

### После
- Ввод дополнительного функционала многопоточности: synchronized, wait, notify, yield, join
- Реализация игры змейка и драйвера экрана
- Реализация компонент(ассемблер, библиотека кодогенерации и RTOS) для платформы STM8
...
### Улучшения компилятора и ассемблера

**Назначение:** Повышение надёжности, производительности и удобства использования инструментов трансляции.

**Ключевые изменения:**

* **Добавлен тихий режим (`-q`)** для интеграции в CI/CD-процессы
* **Улучшена обработка ошибок** — более точные сообщения об ошибках, проверка типов, обработка исключений
* **Оптимизация кода** — свёртка константных выражений, оптимизация инструкций, удаление неиспользуемых меток
* **Рефакторинг CodeGenerator** — переход на объект `Platform`, улучшенная работа с аккумулятором, доработка FIXED-типов
* **Доработан механизм обработки исключений** — трассировка стека, проверка переполнений, улучшенная диагностика

### Переработка системы загрузчиков (j8bf / j8bmb)

**Назначение:** Гибкая и надёжная работа с устройствами через UART-загрузчик.

**Новые возможности:**

* **Поддержка двух режимов связи:**
  * Однопроводной режим (ранее реализованный)
  * Двухпроводной режим с раздельными каналами команд и данных (для удобства использования широкораспространенных плат типа Arduino и т.п.)
* **Интерактивный режим (`-I`)** — терминал для обмена данными с устройством после прошивки
* **Обработка трассировки исключений** — парсинг и вывод стектрейсов из `.dbg`-файлов
* **Улучшенный процесс подключения** — прогресс-индикатор, параметр ожидания устройства (`-w`)
* **Гибкая конфигурация портов** — поддержка форматов `PB0` (однопроводной) и `PB0/PB1` (двухпроводной)

### Обновление бутлоадера (boot/bldr.asm)

**Назначение:** Более надёжный и гибкий загрузчик для AVR-устройств.

**Изменения:**

* **Заменена процедура ожидания перехода в бутлоадер на watchdog-таймер** (1 секунда) для запуска
* **Добавление векторной таблицы процедур** в начале загрузчика для использования из основной программы
* **Поддержка двухпинового режима** — раздельные пины для приёма и передачи

### Рефакторинг инфраструктуры сборки (Maven)

**Назначение:** Упрощение структуры проекта и гибкая конфигурация под разные аппаратные платформы.

**Ключевые изменения:**

* **Упрощение структуры** — исходники перемещены в `src/` вместо `src/main/j8b/`
* **Добавление Maven-профилей** — `default`, `1w`, `arduino-uno` для разных конфигураций
* **Обновление плагина j8b-maven-plugin** — общая логика компиляции, поддержка новых параметров (`targetStdio`, `verboseOutput`)
* **Новые свойства сборки** — `j8b.packaging.version`, `j8b.plugin.version`, `j8b.targetStdio`

### Расширение стандартной библиотеки и ядра системы

**Назначение:** Расширение стандартной библиотеки и гибкое ядро для embedded-разработки.

**Нововведения:**

* **Новые классы** — `IOException`, `TimeoutException`, `NativeException`, `Primitive`, `Math`
* **Обновлённые нативные привязки** — методы ввода (`System.hasInput()`, `System.in()`)
* **Режим разработки (`OS_FT_DEV_MODE`)** — доступ к процедурам загрузчика, отладочный функционал, возможность программной перезагрузки МК
* **Переработанная подсистема STDIO** — отдельные модули `stdin.asm`, `stdout.asm`, поддержка двухпинового режима
* **Кольцевой буфер ввода** — улучшенная работа с асинхронным вводом
* **Обновлённые драйверы устройств** — поддержка atmega168p, atmega328pb
* **Структурированные файлы конфигурации фич** — `features.def`, `ftdepchck.asm`
* **Обновлённые параметры RTOS** — добавлен `DEVEL_MODE`, переименован `STDOUT_PORT` → `STDIO_PORT`

### Улучшения математических функций и низкоуровневых модулей

**Назначение:** Корректная и эффективная работа с арифметикой и памятью.

**Изменения:**

* **Исправление ошибок в функциях деления и умножения**
* **Обработка переполнения в `mul32`**
* **Оптимизация умножения/деления на степени двойки**
* **Новый модуль `circ_buffer.asm`** для работы с кольцевыми буферами

### Компексное тестирование тулкита

**Назначение:** Функциональное тестирование, на основе примеров, на реальном железе и исправление ошибок и недочетов

**Изменения:**

* **Выполнен ряд тестов** на МК ATmega168p и ATmega328p (ARDUINO-UNO)
* **Оттестирована работа 1w и 2w режимов** коммуникации с МК
* **Расширен функционал примеров** для расширенного тестирвоания и информативности
* **Выполнен багфиксинг** RTOSS функций и работ компилятора и ассемблера


### Итог

Проведена комплексная модернизация экосистемы vm5277, затрагивающая все уровни стека разработки:

* **Инструменты трансляции** стали надёжнее и гибче
* **Система загрузки** поддерживает современные сценарии разработки и отладки
* **Инфраструктура сборки** упрощена и адаптирована под разные платформы
* **Стандартная библиотека и ядро** обогащены новыми возможностями и исправлениями

**Можно приступать к первым шагам для знакомства с тулкитом, лучше всего использовать ATmega168p или ATmega328p (как вариант - ARDUINO-UNO)**

### Следующие шаги
- Тестирование на основе примеров с последующим багфиксингом и доработкой RTOS
- Доработка документации



## [2026-01-14] Новые инструменты разработки, рефакторинг и расширение библиотеки common, новый основной загрузчик, Maven-архетипы, документация и мелкий багфиксинг

### ✅ Статус

**Реализован комплекс вспомогательных инструментов для удобной работы с проектом vm5277:**
* **j8bf (flasher)** – утилита для прошивки микроконтроллеров через UART-загрузчик
* **начат IDE-плагин** для NetBeans – предварительная поддержка синтаксиса J8B
* **начат LSP-сервер** – основа для языковой поддержки в редакторах
* **Maven-инструменты** – архетип, плагин и упаковка для сборки проектов
* **j8bmb (mkboot)** – генератор защищённых загрузчиков с уникальными идентификаторами

**Проведён масштабный рефакторинг библиотеки common с целью унификации и расширения функционала:**
* **Лексер** выделен в отдельный универсальный модуль с поддержкой J8B и ассемблера
* **Реализована поддержка протокола загрузчика** (bootloader)
* **Улучшена работа с прошивками** (Intel HEX парсер)
* **Создан легковсный парсер** файлов определений (.equ/.set)

**Создана улучшенная система загрузчиков для AVR с поддержкой аппаратной защиты и нового протокола.**

**Maven примеры:**  
Все примеры из директории examples/j8b переведены в структуру Maven-проектов с использованием архетипа j8b-archetype.  
Теперь каждый пример представляет собой отдельный Maven-проект со стандартной структурой:

### Утилита прошивки j8bf

**Назначение:** Загрузка прошивки в микроконтроллер через UART-загрузчик.

**Ключевые возможности:**
* **Поддержка нескольких платформ:** AVR, STM8, PIC (через параметр <platform>:<mcu>)
* **Умная прошивка (smart mode)** – запись только изменённых страниц Flash
* **Зашифрованные** прошивки – опция -e/--encrypted
* **Автоматическое определение последовательного порта**

### IDE-плагин для NetBeans (черновая версия)

**Назначение:** Базовая интеграция языка J8B в среду NetBeans.

**Реализовано:**
* **MIME-резолвер** для расширений .j8b и .asm
* **Лексер** с адаптером к LSP-серверу
* **Токенизация** с поддержкой всех категорий токенов J8B

### LSP-сервер (начальная реализация)

**Назначение**: Языковой сервер для поддержки J8B в редакторах через LSP.

**Компоненты:**
* **J8BLanguageServer** – сервер для J8B
* **ASMLanguageServer** – сервер для ассемблеров
* **Интерфейс LanguageServer** – общий контракт
* **LSPToken** – передача токенов от лексера к IDE

### Maven-инструменты

**Назначение:** Интеграция сборки проектов J8B в экосистему Maven.

**Архетип проекта (j8b-archetype)**
* Генерация шаблона проекта J8B
* Стандартная структура Maven + директория src/main/j8b
* Готовый pom.xml с подключённым плагином

**Плагин Maven (j8b-maven-plugin)**
* цель j8b:compile – компиляция J8B → ассемблер → HEX
* цель j8b:run – прошивка на устройство (запуск j8bf)
* цель j8b:package – (заглушка)
* цель j8b:install – (заглушка)

### Генератор загрузчиков j8bmb

**Назначение:** Создание уникальных защищённых загрузчиков с аппаратной привязкой.
**Ключевые возможности:**
* Генерация уникального идентификатора (UID) для каждого МК
* Поддержка шифрования прошивки
* Кастомизация пина для коммуникации с загрузчиком
* Сборка загрузчика с подставленными параметрами через ассемблер

### Инфраструктура
* Централизованный репозиторий Maven – https://vm5277.ru/maven-repo
* Автоматическая загрузка тулкита – при отсутствии в системе

### Новая система лексического анализа
**Ключевые возможности:**
* Поддержка двух режимов: J8B и ASM
* Расширяемая система ключевых слов через наследование
* Точное позиционирование в исходном коде
* Эффективное чтение с буферизацией

### Парсер файлов определений (DefReader.java)
**Назначение:** Чтение и парсинг файлов с директивами .equ, .set, .def.

**Поддерживаемые конструкции:**
* **Числовые константы:** .equ FLASH_SIZE = 0x2000
* **Поддержка алиасов:** .EQU SPMCR = SPMCSR

### Поддержка протокола загрузчика
**Назначение:** Унифицированный протокол общения с загрузчиком.
**Классы**:
* BldrType – типы загрузчиков (TINY, NORMAL, ...)
* BldrRequest – типы запросов: 	INFO, REBOOT, PAGE_ERASE, ...
* BldrResult – результаты операций: OK, IDENTICAL, NOT_EQUAL, ...


### Итог

Создан полноценный набор инструментов для комфортной разработки под vm5277:
* **Локальная разработка** – Maven + архетип
* **Прошивка устройств** – умный flasher с поддержкой шифрования
* **IDE-поддержка** – начало интеграции в NetBeans
* **Уникальные загрузчики** – генерация с аппаратной привязкой
* **Будущая расширяемость** – LSP-сервер для поддержки в VS Code, Eclipse, etc.
* **Универсальный лексер** – основа для компилятора, ассемблера, IDE, LSP

*Инструменты готовы к альфа-тестированию и будут дорабатываться по мере развития проекта.*
...
## [2025-12-10] - Багфиксинг механизма исключений

### Исправления и улучшения:
- **Динамические ID исключений:** Удалены статические константы, теперь ID определяются через VarType.getExceptionId()
- **Оптимизация catch блоков:** Код генерируется только если исключения действительно попадают в catch-блок
- **Проверка throws методов:** Добавлена проверка неперехваченных исключений при вызове методов
- **Улучшенная трассировка:** Добавлены методы для работы со свойствами исключений в кодогенераторе
- **Рефакторинг структур:** Улучшена работа с иерархией исключений
