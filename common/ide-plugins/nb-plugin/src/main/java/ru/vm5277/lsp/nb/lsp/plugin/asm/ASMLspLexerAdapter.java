...
import ru.vm5277.lsp.nb.lsp.plugin.InputStreamAdapter;
...
public class ASMLspLexerAdapter implements Lexer<ASMTokenId> {
    ...
    public ASMLspLexerAdapter(LexerRestartInfo<ASMTokenId> info) {
		...
		if(null==server) {
			server = new ASMLanguageServer(PlatformType.AVR, "atmega328"); //TODO надо придумать как определять текущую платформу и конкретный МК
	...
	public static ASMLanguageServer getLSPServer() {
		return server;
	}
