...
public class Main {
	...
	public static int exec(String[] args) {
		...
		if(0x02<=args.length) {
			...
			try{
				cpuFreqKHz = Float.parseFloat(args[0x01]);
				if(0>=cpuFreqKHz || 255<cpuFreqKHz) {
			...
			if(0x03<=args.length) {
				...
				for(int i=(detectMode ? 2 : 3); i<args.length; i++) {
					String arg = args[i];
					...
					else if(!detectMode && args.length>i+1 && (arg.equals("-f") || arg.equals("--fwversion"))) {
						try {
					...
					else if(!detectMode && (arg.equals("-s") || arg.equals("--smart"))) {
						smartMode = true;
					}
					else if(arg.equals("-w") || arg.equals("--wait")) {
						waitTime = WAIT_TIME;
					}
					else if(!detectMode && (arg.equals("-C") || arg.equals("--clear-cache"))) {
						doClearCache = true;
					}
					else if(!detectMode && (arg.equals("-E") || arg.equals("--erase"))) {
						doErase = true;
					}
					else if(!detectMode && (arg.equals("-F") || arg.equals("--flash"))) {
						doFlash = true;
					}
					else if(!detectMode && (arg.equals("-V") || arg.equals("--verify"))) {
						doVerify = true;
					}
					else if(!detectMode && (arg.equals("-R") || arg.equals("--reboot"))) {
						doReboot = true;
					}
					else if(arg.equals("-q") || arg.equals("--quiet")) {
						quiet = true;
					}
					else if(!detectMode && (arg.equals("-e") || arg.equals("--encrypted"))) {
						isEncryptedFw = true;
					}
					else if(!detectMode && (arg.equals("-I") || arg.equals("--interactive"))) {
		...
		if(!detectMode && (null==platform || null==mcu || mcu.isEmpty())) {
			showInvalidDeviceFormat(args[0]);
			return 1;
		}

		Path defsPath = toolkitPath.resolve("defs").normalize();
		if(!defsPath.toFile().exists()) {
			showInvalidDir(defsPath);
			return 1;
		}

		if(detectMode) {
			String deviceInfo = detectDevice(toolkitPath, cpuFreqKHz, waitTime, retries, quiet);
			if(null!=deviceInfo) {
			...
			Path incPath = defsPath.resolve(platform.name().toLowerCase()).resolve(mcu + ".asm");
			if(!incPath.toFile().exists()) {
				showInvalidDir(incPath);
				return 1;
			}

			Path rtosPath = toolkitPath.resolve("rtos").resolve(platform.name().toLowerCase()).normalize();
			if(!rtosPath.toFile().exists()) {
				showInvalidDir(rtosPath);
				return 1;
			}

			Path defPath = rtosPath.resolve("devices").resolve(mcu + ".def");
			if(!defPath.toFile().exists()) {
				showInvalidDir(defPath);
				return 1;
			}
			...
			BootloaderIface bIface = new BootloaderIface(cpuFreqKHz, serialDeviceName, waitTime, retries);
			if(bIface.open()) {
				boolean result = true;
				System.out.println();
	...
	public static String detectDevice(Path toolkitPath, float cpuFreqKHz, int waitTime, int retries, boolean quiet) {
		String result = null;
		BootloaderIface bIface = new BootloaderIface(cpuFreqKHz, null, waitTime, retries);
		if(bIface.open()) {
			String platformName = bIface.getDeviceInfo().getPlatform().name().toLowerCase();
			Path signaturesPath = toolkitPath.resolve("defs").resolve(platformName).resolve("signatures.txt").normalize();
			if(signaturesPath.toFile().exists()) {
				try (Scanner scan = new Scanner(signaturesPath.toFile())) {
					while (scan.hasNextLine()) {							
						String parts[] = scan.nextLine().split("\\s+", 0x02);
						if(0x02==parts.length) {
							try {
								Integer signature = Integer.parseInt(parts[0x00], 0x10);
								if(bIface.getDeviceInfo().getSignature()==signature) {
									result = platformName + ":" + parts[0x01].trim() + " " + bIface.getDeviceInfo().getStdioStr();
									if(!quiet) {
										System.out.println();
										System.out.println("Serial port: " + (null==bIface.getSerialPort() ? "null" : bIface.getSerialPort().getPortName()));
										System.out.println("Connection mode:" + (bIface.getDeviceInfo().isSingleWireMode() ? "1" : "2") + " wire mode");
		...
		if(null==result && null!=bIface.getDeviceInfo() && !quiet) {
			System.err.println(	"[ERROR] Unknown device, platform:" + bIface.getDeviceInfo().getPlatform().name() +
								", signature: 0x" + String.format("%08x", bIface.getDeviceInfo().getSignature()));
		}
		try {bIface.close();} catch(Exception ignored) {}
		return result;
	}
	...
	private static void showDisclaimer() {
		System.out.println("Flasher v." + VERSION + ": firmware flasher for vm5277 Embedded Toolkit");
	}

	private static void showHelp() {
		System.out.println("Flasher: firmware flasher for vm5277 Embedded Toolkit");
		...
		System.out.println("       j8bf" + (isWindows ? ".exe" : "") + " detect <freq> [options]");
		System.out.println();
		System.out.println("Commands:");
		System.out.println("  detect <freq>           Auto-detect connected device");
		System.out.println("                          Scans serial ports to identify MCU type and returns");
		System.out.println("                          target in format <platform>:<mcu> (e.g., avr:atmega168p)");
		System.out.println("                          Use this to discover unknown device before flashing");
		System.out.println();
		...
		System.out.println("                          (stdin/stdout redirected to serial port, Esc for exit)");
		...
		System.out.println("  j8bf detect 16 -S /dev/ttyUSB0");
		System.out.println("  j8bf detect 9.6");
	}
	...
	private static void showInvalidDir(Path subPath) {
		System.err.println("[ERROR] Directory not found or empty:" + subPath.toString());
	}
