...
//		String source = "class A{ int t3=7==10-6/2?7:2;}";
		...
		assertEquals("i1", toStringValue(((VarFieldExpression)e3.getRight()).getValue()));
		assertEquals(Operator.PLUS, e2.getOperator());
		assertEquals(true, e2.getRight()instanceof VarFieldExpression);
		assertEquals("i2", toStringValue(((VarFieldExpression)e2.getRight()).getValue()));
		assertEquals(Operator.MINUS, e1.getOperator());
		assertEquals(true, e1.getRight()instanceof VarFieldExpression);
		assertEquals("i3", toStringValue(((VarFieldExpression)e1.getRight()).getValue()));
