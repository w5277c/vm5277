...
import ru.vm5277.common.RTOSHaltMode;
...
public class Generator extends CodeGenerator {
	...
	public void constToAcc(CGScope scope, int size, long value, boolean isFixed) throws CompileException {
		...
		if(accum.getSize()>2) scope.append(new CGIAsmLd("ldi", "r18", Long.toString(value&0xff))); value >>= 0x08;
		if(accum.getSize()>3) scope.append(new CGIAsmLd("ldi", "r19", Long.toString(value&0xff)));
	}
	...
	public void accToArrReg(CGScope scope) throws CompileException {
		...
		scope.append(new CGIAsmMv("mov", "r16", "r26"));
		scope.append(new CGIAsmMv("mov", "r17", "r27"));
	}

	@Override
	public void arrToCells(CGScope scope, CGArrCells arrCells, CGCells cells, CGExcs excs) throws CompileException {
		if(VERBOSE_LO <= verbose) scope.append(new CGIText(";arr '" + arrCells + "'->cells " + cells));
		
		computeArrCellAddr(scope, new CGCells(CGCells.Type.ACC), arrCells, excs);
		...
		switch(cells.getType()) {
			case REG: //TODO при size==1, атомарность не нужна
				for(int i=0; i<size; i++) scope.append(new CGIAsmLd("ld", "r" + cells.getId(i), "x" + (i!=size-1 ? "+" : "")));
				break;
			case ARGS:
			case STACK_FRAME:
	...
	public void arrRegToCells(CGScope scope, CGCells cells) throws CompileException {
		...
		switch(cells.getType()) {
			case ACC:
				...
				break;
			case REG:
				scope.append(new CGIAsmMv("mov", "r" + cells.getId(0), "r26"));
				scope.append(new CGIAsmMv("mov", "r" + cells.getId(1), "r27"));
				break;
			case ARGS:
			case STACK_FRAME:
				CGIContainer _cont = new CGIContainer();
...
//			if(accum.getSize()<fScope.getCells().getSize()) accum.setSize(fScope.getCells().getSize());
	...
	public void cellsToRegs(CGScope scope, CGCells cells, byte[] registers) throws CompileException {
		...
		switch(cells.getType()) {
			...
			case ACC:
				...
				for(int i=0; i<size; i++) {
					if(accRegs[i]!=registers[i]) {
						scope.append(new CGIAsmMv("mov", "r" + registers[i], "r" + accRegs[i]));
					}
				}
				break;
			case REG:
			...
			case HEAP_ALT:
				...
				for(int i=0; i<size; i++) {
					scope.append(new CGIAsmLd("ld", "r" + registers[i], "x+"));
				}
				break;
			case STAT:
				for(int i=0; i<size; i++) {
					scope.append(new CGIAsmLd("lds", "r" + registers[i], "_OS_STAT_POOL+" + cells.getId(i)));
				}
				break;
			default:
				throw new CompileException("Unsupported cell type:" + cells.getType());
	...
	public CGIContainer constToCells(CGScope scope, long value, CGCells cells, boolean isFixed) throws CompileException {
		...
		if(0x04==accum.getSize() && CGCells.Type.REG!=cells.getType()) {
			...
			if(19!=tmpReg) scope.append(new CGIAsmMv("mov", "j8b_atom", "r"+tmpReg));
		}
		...
		if(19!=tmpReg) scope.append(new CGIAsmMv("mov", "r"+tmpReg, "j8b_atom"));
		if(RTOSFeatures.contains(RTOSFeature.OS_FT_MULTITHREADING)) scope.append(new CGIAsm("clt"));
		return cont;
	}
		...
		switch(op) {
			...
			case MULT:
				...
				if(0x01==maxSize) {
					...
					switch(cells.getType()) {
						case REG:
							scope.append(new CGIAsmMv("mov", "r17", "r"+cells.getId(0)));
							break;
						case ARGS:
						case STACK_FRAME:
					...
					switch(cells.getType()) {
						case REG:
							scope.append(new CGIAsmMv("mov", "r18", "r"+cells.getId(0)));
							scope.append(new CGIAsmMv("mov", "r19", "r"+cells.getId(1)));
							break;
						case ARGS:
						case STACK_FRAME:
	...
	@Override
	public void constAction(CGScope scope, Operator op, long k, boolean isFixed, CGExcs excs) throws CompileException {
		...
		switch(op) {
			...
			case SHL: //TODO math overflow check
				...
				if(0x03==octetOffset) {
					scope.append(new CGIAsmMv("mov", "r19", "r16"));
					...
					scope.append(new CGIAsmMv("mov", "r18", "r16"));
					scope.append(new CGIAsmMv("mov", "r19", "r17"));
					scope.append(new CGIAsmLd("ldi", "r17", "0x00"));
					scope.append(new CGIAsmLd("ldi", "r16", "0x00"));
				}
				else if(0x01==octetOffset) {
					scope.append(new CGIAsmMv("mov", "r19", "r18"));
					scope.append(new CGIAsmMv("mov", "r18", "r17"));
					scope.append(new CGIAsmMv("mov", "r17", "r16"));
					scope.append(new CGIAsmLd("ldi", "r16", "0x00"));
				}
			...
			case SHR:
				octetOffset = (int)k/8;
				if(0x03==octetOffset) {
					scope.append(new CGIAsmMv("mov", "r16", "r19"));
					scope.append(new CGIAsmLd("ldi", "r17", "0x00"));
					scope.append(new CGIAsmLd("ldi", "r18", "0x00"));
					scope.append(new CGIAsmLd("ldi", "r19", "0x00"));
				}
				else if(0x02==octetOffset) {
					scope.append(new CGIAsmMv("mov", "r16", "r18"));
					scope.append(new CGIAsmMv("mov", "r17", "r19"));
					scope.append(new CGIAsmLd("ldi", "r18", "0x00"));
					scope.append(new CGIAsmLd("ldi", "r19", "0x00"));
				}
				else if(0x01==octetOffset) {
					scope.append(new CGIAsmMv("mov", "r16", "r17"));
					scope.append(new CGIAsmMv("mov", "r17", "r18"));
					scope.append(new CGIAsmMv("mov", "r18", "r19"));
					scope.append(new CGIAsmLd("ldi", "r19", "0x00"));
				}
				...
				if(0!=k) {
					...
					scope.append(new CGIAsmCall(CALL_INSTR, "os_div" + (accum.getSize()*8) + "p2_x" + (1<<k), true));
				}
				break;
			case MULT:
...
//	Размер аккумулятора может быть меньше чем cells
		...
		for(int i=accum.getSize()-1; i>=0; i--) {
			...
			if(i>=cells.getSize()) {
				...
				switch(cells.getType()) {
					...
					case ARRAY:
						if(i==accum.getSize()-1 && 19!=tmpReg) cont.append(new CGIAsm("push", "r" + tmpReg));
						cont.append(new CGIAsmLd("ld", "r" + tmpReg, "-x"));
						cont.append(new CGIAsm("cp", "r" + racc + ",r" + tmpReg));
						if(0==i && 19!=tmpReg) cont.append(new CGIAsm("pop", "r" + tmpReg));
						break;
					default:
		...
		switch(op) {
			case LT:
				if(isOr) { // В связке с OR
					cont.append(new CGIAsmCondJump("brcs", branchScope.getEnd())); // True
					...
					cont.append(new CGIAsmCondJump("brcc", branchScope.getEnd())); // False
				}
				break;
			case GTE:
				if(isOr) { // В связке с OR
					...
					if(0!=index) {  // Старшие байты
						cont.append(new CGIAsmCondJump("brne", lbScope)); // Если < - продолжить
						lbScope.setUsed();
					}
				}
				else { // В связке с AND 
			...
			case NEQ:
				if(isOr) { // В связке с OR
					cont.append(new CGIAsmCondJump("brne", branchScope.getEnd()));
				}
				else { // В связке с AND 
					if(0!=index) {  //Старшие байты
						cont.append(new CGIAsmCondJump("brne", lbScope));
	...
	public void invokeInterfaceMethod(CGScope scope, String className, String methodName, VarType type, VarType[] types, VarType ifaceType, int methodSN)																										throws CompileException {
		...
		scope.append(new CGIAsmLd("ldi", "r17", Long.toString(methodSN)));
...
/*		int argsSize = 0;
	...
	public void updateClassRefCount(CGScope scope, CGCells cells, boolean isInc) throws CompileException {
		...
		switch(cells.getType()) {
			case REG:
				scope.append(new CGIAsmMv("mov", "r30", "r" + cells.getId(0x00)));
				if(0x01 == getRefSize()) {
					scope.append(new CGIAsmLd("ldi", "r31", "0x00"));
				}
				else {
					scope.append(new CGIAsmMv("mov", "r31", "r" + cells.getId(0x01)));
				}
				break;
			case ARGS:
			case STACK_FRAME:
			...
			case STAT:
				if(0x01 == getRefSize()) {
					scope.append(new CGIAsmLd("lds", "r30", "_OS_STAT_POOL" + cells.getId(0)));
					scope.append(new CGIAsmLd("ldi", "r31", "0x00"));
				}
				else {
					scope.append(new CGIAsmLd("lds", "j8b_atom", "_OS_STAT_POOL+" + cells.getId(0)));
					scope.append(new CGIAsmLd("lds", "r31", "_OS_STAT_POOL+" + cells.getId(1)));
					scope.append(new CGIAsmMv("mov", "r30", "j8b_atom"));
				}
				break;
			default:
		...
		scope.append(new CGIAsmCall(CALL_INSTR, "j8bproc_class_refcount_" + (isInc ? "inc" : "dec"), true));
		if(RTOSFeatures.contains(RTOSFeature.OS_FT_MULTITHREADING)) scope.append(new CGIAsm("clt"));
	}
	...
	public void eInstanceof(CGScope scope, VarType type) throws CompileException {
		...
		if(0x04==accum.getSize()) scope.append(new CGIAsmMv("mov", "j8b_atom", "r19"));
		...
		if(0x04==accum.getSize()) scope.append(new CGIAsmMv("mov", "r19", "j8b_atom"));
	...
	public void terminate(CGScope scope, boolean systemStop, boolean excsCheck) throws CompileException {
		...
		if(RTOSHaltMode.BLINK_N_RESET==okMode || RTOSHaltMode.BLINK_N_RESET==errMode) {
