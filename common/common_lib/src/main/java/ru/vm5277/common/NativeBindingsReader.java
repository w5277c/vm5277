...
public class NativeBindingsReader {
	...
	public NativeBindingsReader(Path rtosPath) throws CompileException {
		try(BufferedReader br = new BufferedReader(new FileReader(rtosPath.resolve("binding.cfg").normalize().toFile()))) {
			while(true) {
				...
				if(!line.isEmpty()) {
					try {
						...
						throw new CompileException(e.getMessage() + " at line:" + line);
					}
				}
			}
		}
		catch(Exception ex) {
			throw new CompileException("Cannot read native bindings map file:" + rtosPath + ", exception:" + ex.getMessage());
		}
