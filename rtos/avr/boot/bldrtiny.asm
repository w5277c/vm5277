...
;Запросы  (от хоста)
	...
	.EQU	REQ_BLDR_REBOOT							= 0x01	;Переход на основную программу
...
;Остальные запросы тел не имеют
	...
	.EQU	BLDR_VERSION							= 0b00000000 | 0x00000000 ;7,6 - тип, 5-0 - версия
	.EQU	BLDR_START_WADDR						= BOOT_256W_ADDR
...
.IF FLASH_PAGESIZE > 128
	.ERROR "FLASH PAGE SIZE >128 not supported"
.ENDIF
.IF (SRAM_START+_BLDR_BUFFER_SIZE)>0x100
	.ERROR "Insufficient RAM for buffer"
.ENDIF
...
.ENDIF
	...
	STS SPL,ACCUM_L // можно заменить на OUT, но SPL имеет адрес+20
...
;---PAGE-WRITE----------------------------------------------
	LDD ZH,Y+0x04
	LDD ZL,Y+0x05
	LDI YL,SRAM_START+_BLDR_PROTOCOL_HEADER_SIZE-0x01+0x02
...
;-----------------------------------------------------------
	...
	ST Y+,C0x00
	ST Y+,C0x00
	LDI XL,_BLDR_PROTOCOL_HEADER_SIZE-0x01
...
;OUT: XL-длина данных, TEMP_EH-XORSUM, XH=0xff-ошибка приема
;-----------------------------------------------------------
	LDI YL,SRAM_START										;Устанавливаем адрес буфера
	LDI XL,0x00												;Длина принятых данных
...
BLDR_UART_RECV_NR__GOT_BYTE:
	CPI XL,0xff												;Признак переполнения буфера
...
_BLDR_UART_RECV_NR__NEXT:
	...
	INC XL													;Инкрементируем счетчик длины данных
...
;IN: XL-размер данных без XORSUM (не может быть 0)
;-----------------------------------------------------------
.IFDEF STDIO_PORT_REGID
...
.ELSE
	...
	SBI STDOUT_PORT_REGID,STDOUT_PINNUM
.ENDIF
	;Нельзя просто прыгнуть на вектор прерывания RESET
	;Так как там прописан адрес нашего бутлоадера
	;Нам нужно прыгнуть на _OS_INIT
	LDI_Z _OS_INIT
	IJMP
