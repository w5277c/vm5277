...
;-----------------------------------------------------------------------------------------------------------------------
	...
	.DEVICE		ATmega328
	.MNEMONICS	ADD,ADC,ADIW,SUB,SUBI,SBC,SBCI,SBIW,AND,ANDI,OR,ORI,EOR,COM,NEG,SBR,CBR,INC,DEC,CLR,SER,MUL,MULS,MULSU,FMUL,FMULSU
	.MNEMONICS	RJMP,IJMP,JMP,RCALL,ICALL,CALL,RET,RETI,CPSE,CP,CPC,CPI,SBRC,SBRS,SBIC,SBIS
	.MNEMONICS	BRBS,BRBC,BREQ,BRNE,BRCS,BRCC,BRSH,BRLO,BRMI,BRPL,BRGE,BRLT,BRHS,BRHC,BRTS,BRTC,BRVC,BRIE,BRID
	.MNEMONICS	SBI,CBI,LSL,LSR,ROL,ROR,ASR,SWAP,BSET,BCLR,BST,BLD,SEC,CLC,SEN,CLN,SEZ,CLZ,SEI,CLI,SES,CLS,SEV,CLV,SET,CLT,SEH,CLH
	.MNEMONICS	MOV,MOVW,LDI,LDX,LDXP,LDMX,LDY,LDYP,LDMY,LDZ,LDZP,LDMZ,LDDY,LDDZ,LDS
	.MNEMONICS	STX,STXP,STMX,STY,STYP,STMY,STZ,STZP,STMZ,STDY,STDZ,STS,LPM,LPMZ,LPMZP,SPM,IN,OUT,PUSH,POP
	.MNEMONICS	NOP,SLEEP,WDR,BREAK
...
;---CONSTANTS-----------------------------------------------
	...
	.EQU	_CORE_IR_QNT							= 0x1a	;Количество перырваний(без RESET)
...
.ORG 0x0000
	JMP		_CORE_INIT										; Reset Handler
	CALL	_CORE_IR_HANDLER								; External Interrupt0 Handler
	CALL	_CORE_IR_HANDLER								; External Interrupt1 Handler
	CALL	_CORE_IR_HANDLER								; Pin Change Interrupt Request 0
	CALL	_CORE_IR_HANDLER								; Pin Change Interrupt Request 1
	CALL	_CORE_IR_HANDLER								; Pin Change Interrupt Request 2
	CALL	_CORE_IR_HANDLER								; Watchdog Time-out Interrupt
	CALL	_CORE_IR_HANDLER								; Timer/Counter2 Compare Match A
	CALL	_CORE_IR_HANDLER								; Timer/Counter2 Compare Match B
	CALL	_CORE_IR_HANDLER								; Timer/Counter2 Overflow
	CALL	_CORE_IR_HANDLER								; Timer/Counter1 Capture Event
	CALL	_CORE_IR_HANDLER								; Timer/Counter1 Compare Match A
	CALL	_CORE_IR_HANDLER								; Timer/Counter1 Compare Match B
	CALL	_CORE_IR_HANDLER								; Timer/Counter1 Overflow
	JMP		_CORE_TIMER_A_HANDLER							; Timer/Counter0 Compare Match A
	JMP		_CORE_IR_HANDLER								; Timer/Counter0 Compare Match B
	CALL	_CORE_IR_HANDLER								; Timer/Counter0 Overflow
	CALL	_CORE_IR_HANDLER								; SPI Serial Transfer Complete
	CALL	_CORE_IR_HANDLER								; USART Rx Complete
	CALL	_CORE_IR_HANDLER								; USART, Data Register Empty
	CALL	_CORE_IR_HANDLER								; USART, Tx Complete
	CALL	_CORE_IR_HANDLER								; ADC Conversion Complete
	CALL	_CORE_IR_HANDLER								; EEPROM Ready
	CALL	_CORE_IR_HANDLER								; Analog Comparator
	CALL	_CORE_IR_HANDLER								; 2-wire Serial Interface
	CALL	_CORE_IR_HANDLER								; Store Program Memory Ready
